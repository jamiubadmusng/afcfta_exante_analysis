...
# ====================== Industry-level analysis for IGAD =======================
...
# ----- Data prep (merge + IGAD border variables; ifelse for dummies, time-varying via as.integer(factor())) -----
prepare_data <- function(itpde_file) {
  ...
  df <- itpde %>%
    ...
    mutate(
      igad = ifelse(exporter %in% iga_countries & importer %in% iga_countries & exporter != importer, 1, 0),
      igad_to_af = ifelse(exporter %in% iga_countries & importer %in% afc_countries & !(importer %in% iga_countries) & exporter != importer, 1, 0),
      af_to_igad = ifelse(importer %in% iga_countries & exporter %in% afc_countries & !(exporter %in% iga_countries) & exporter != importer, 1, 0),
      af_n_igad  = ifelse(exporter %in% afc_countries & importer %in% afc_countries & !(exporter %in% iga_countries) & !(importer %in% iga_countries) & exporter != importer, 1, 0),
      igad_to_row = ifelse(exporter %in% iga_countries & !(importer %in% afc_countries) & exporter != importer, 1, 0),
      af_n_igad_to_row = ifelse(exporter %in% afc_countries & !(exporter %in% iga_countries) & !(importer %in% afc_countries) & exporter != importer, 1, 0),
      rr_n_igad = ifelse(!(exporter %in% afc_countries) & !(importer %in% afc_countries) & exporter != importer, 1, 0)
    ) %>%
    mutate(
      igad            = as.integer(factor(paste(igad, year, sep = "_"))),
      igad_to_af      = as.integer(factor(paste(igad_to_af, year, sep = "_"))),
      af_to_igad      = as.integer(factor(paste(af_to_igad, year, sep = "_"))),
      af_n_igad       = as.integer(factor(paste(af_n_igad, year, sep = "_"))),
      igad_to_row     = as.integer(factor(paste(igad_to_row, year, sep = "_"))),
      af_n_igad_to_row= as.integer(factor(paste(af_n_igad_to_row, year, sep = "_"))),
      rr_n_igad       = as.integer(factor(paste(rr_n_igad, year, sep = "_"))),
...
# ----- Estimation (IGAD model) -----
estimate_model <- function(df, industry_id, industry_descr) {
  m <- fepois(
    trade ~ cntg + col + dist + lang + rta + rr_n_igad +
      igad + igad_to_af + af_to_igad + af_n_igad + af_n_igad_to_row + igad_to_row |
      exp_year + imp_year,
    data = df,
    vcov = ~ pair_id
  )
  ...
  broom::tidy(m, conf.int = TRUE) %>%
    filter(term %in% c("igad_to_af", "af_to_igad")) %>%
...
write_csv(results, file.path("C:/Users/muham/EGEI Dissertation/afcfta/output/csv/igad_border_results.csv"))
...
# ----- Make the two plots -----
make_rank_plot(results, "igad_to_af", "graph_igad_to_af.pdf",
               ylab = "IGAD_to_AfCFTA Border Effect",
               sig_only = FALSE,      
               alpha = 0.10,
               se_pctl_max = 0.95,    
               ciwidth_pctl_max = 0.95,  
...
make_rank_plot(results, "af_to_igad", "graph_af_to_igad.pdf",
               ylab = "AfCFTA_to_IGAD Border Effect",
               sig_only = FALSE,      
               alpha = 0.10,
               se_pctl_max = 0.95,    
               ciwidth_pctl_max = 0.95,  
