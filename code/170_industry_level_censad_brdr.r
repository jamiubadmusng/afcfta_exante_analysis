# ====================== Industry-level analysis for CEN-SAD =======================
...
# ----- Data prep (merge + CEN-SAD border variables) -----
...
# ----- Estimation (CEN-SAD model) -----
estimate_model <- function(df, industry_id, industry_descr) {
  m <- fepois(
    trade ~ cntg + col + dist + lang + rta + rr_n_cen +
      cen + cen_to_af + af_to_cen + af_n_cen + af_n_cen_to_row + cen_to_row |
      exp_year + imp_year,
    data = df,
    vcov = ~ pair_id
  )
  ...
  broom::tidy(m, conf.int = TRUE) %>%
    filter(term %in% c("cen_to_af", "af_to_cen")) %>%
...
write_csv(results, file.path("C:/Users/muham/EGEI Dissertation/afcfta/output/csv/censad_border_results.csv"))
...
make_rank_plot(results, "cen_to_af", "graph_cen_to_af.pdf",
               ylab = "CEN-SAD_to_AfCFTA Border Effect",
               sig_only = FALSE,
               se_pctl_max = 0.95,
               ciwidth_pctl_max = 0.95,
               est_pctl_range = c(0.05, 0.95))

make_rank_plot(results, "af_to_cen", "graph_af_to_cen.pdf",
               ylab = "AfCFTA_to_CEN-SAD Border Effect",
               sig_only = FALSE,
               se_pctl_max = 0.95,
               ciwidth_pctl_max = 0.95,
               est_pctl_range = c(0.05, 0.95))
