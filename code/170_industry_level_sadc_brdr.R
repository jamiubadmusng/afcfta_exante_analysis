# ====================== Industry-level analysis for SADC =======================
...
sad_countries <- c("AGO","BWA","COM","COD","SWZ","LSO","MDG","MWI","MUS","MOZ","NAM","ZAF","SYC","TZA","ZMB","ZWE")

# ----- Data prep (merge + SADC border variables; ifelse for dummies, time-varying via as.integer(factor())) -----
prepare_data <- function(itpde_file) {
  ...
  df <- itpde %>%
    ...
    mutate(
      sadc = ifelse(exporter %in% sad_countries & importer %in% sad_countries & exporter != importer, 1, 0),
      sadc_to_af = ifelse(exporter %in% sad_countries & importer %in% afc_countries & !(importer %in% sad_countries) & exporter != importer, 1, 0),
      af_to_sadc = ifelse(importer %in% sad_countries & exporter %in% afc_countries & !(exporter %in% sad_countries) & exporter != importer, 1, 0),
      af_n_sadc  = ifelse(exporter %in% afc_countries & importer %in% afc_countries & !(exporter %in% sad_countries) & !(importer %in% sad_countries) & exporter != importer, 1, 0),
      sadc_to_row = ifelse(exporter %in% sad_countries & !(importer %in% afc_countries) & exporter != importer, 1, 0),
      af_n_sadc_to_row = ifelse(exporter %in% afc_countries & !(exporter %in% sad_countries) & !(importer %in% afc_countries) & exporter != importer, 1, 0),
      rr_n_sadc = ifelse(!(exporter %in% afc_countries) & !(importer %in% afc_countries) & exporter != importer, 1, 0)
    ) %>%
    mutate(
      sadc            = as.integer(factor(paste(sadc, year, sep = "_"))),
      sadc_to_af      = as.integer(factor(paste(sadc_to_af, year, sep = "_"))),
      af_to_sadc      = as.integer(factor(paste(af_to_sadc, year, sep = "_"))),
      af_n_sadc       = as.integer(factor(paste(af_n_sadc, year, sep = "_"))),
      sadc_to_row     = as.integer(factor(paste(sadc_to_row, year, sep = "_"))),
      af_n_sadc_to_row= as.integer(factor(paste(af_n_sadc_to_row, year, sep = "_"))),
      rr_n_sadc       = as.integer(factor(paste(rr_n_sadc, year, sep = "_"))),
...
# ----- Estimation (SADC model) -----
estimate_model <- function(df, industry_id, industry_descr) {
  m <- fepois(
    trade ~ cntg + col + dist + lang + rta + rr_n_sadc +
      sadc + sadc_to_af + af_to_sadc + af_n_sadc + af_n_sadc_to_row + sadc_to_row |
      exp_year + imp_year,
    data = df,
    vcov = ~ pair_id
  )
  ...
  broom::tidy(m, conf.int = TRUE) %>%
    filter(term %in% c("sadc_to_af", "af_to_sadc")) %>%
...
write_csv(results, file.path("C:/Users/muham/EGEI Dissertation/afcfta/output/csv/sadc_border_results.csv"))
...
# ----- Make the two plots -----
make_rank_plot(results, "sadc_to_af", "graph_sadc_to_af.pdf",
               ylab = "SADC_to_AfCFTA Border Effect",
               sig_only = FALSE,
               se_pctl_max = 0.95,
               ciwidth_pctl_max = 0.95,
               est_pctl_range = c(0.05, 0.95))

make_rank_plot(results, "af_to_sadc", "graph_af_to_sadc.pdf",
               ylab = "AfCFTA_to_SADC Border Effect",
               sig_only = FALSE,
               se_pctl_max = 0.95,
               ciwidth_pctl_max = 0.95,
               est_pctl_range = c(0.05, 0.95))
