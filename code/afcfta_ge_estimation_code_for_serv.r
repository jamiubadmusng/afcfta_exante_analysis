...
# for Services Sector data in ITPD-E-R02
...
# Load data
data <- read.dta13("input/afcfta_2019_serv_balanced.dta")
...
# Save processed data
saveRDS(dt, "input/ge_ppml_data_serv.rds")
...
# Load prepared data
dt <- readRDS("input/ge_ppml_data_serv.rds")
...
# Save results
saveRDS(dt, "output/rds/full_static_all_serv.rds")
...
saveRDS(imr_dt, "output/rds/imrs_all_serv.rds")
...
saveRDS(all_indexes, "output/rds/all_indexes_geppml_serv.rds")
...
saveRDS(all_indexes_with_row, "output/rds/all_indexes_with_row_geppml_serv.rds")
...
welfare_plot <- ggplot(all_indexes_with_row, 
  ...
  scale_fill_manual(values = c("gray60", "steelblue"), 
                    ...
                    name = "") +
  labs(title = "Welfare Effects of AfCFTA Agreement for Services Sector",
...
# Save the plot
ggsave("output/figures/welfare_effects_with_row_geppml_serv.pdf", welfare_plot, width = 10, height = 12, dpi = 300)
...
welfare_plot_afcfta <- ggplot(all_indexes_with_row[is_afcfta == TRUE], 
  ...
  geom_text(aes(label = sprintf("%.2f%%", rGDP_full_ch)), 
            ...
            size = 3.5) +
  coord_flip() +
  labs(title = "Welfare Effects for AfCFTA Member Countries for Services Sector",
       subtitle = "% Change in Real GDP",
       x = "Country",
       y = "% Change in Real GDP") +
  theme_minimal()
...
ggsave("output/figures/welfare_effects_afcfta_only_geppml_serv.pdf", welfare_plot_afcfta, width = 8, height = 12, dpi = 300)
