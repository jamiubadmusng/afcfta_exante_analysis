# ====================== Industry-level analysis for EAC =======================
...
# ----- Data prep (merge + EAC border variables; ifelse for dummies, time-varying via as.integer(factor())) -----
prepare_data <- function(itpde_file) {
  ...
  df <- itpde %>%
    ...
    mutate(
      eac = ifelse(exporter %in% eac_countries & importer %in% eac_countries & exporter != importer, 1, 0),
      eac_to_af = ifelse(exporter %in% eac_countries & importer %in% afc_countries & !(importer %in% eac_countries) & exporter != importer, 1, 0),
      af_to_eac = ifelse(importer %in% eac_countries & exporter %in% afc_countries & !(exporter %in% eac_countries) & exporter != importer, 1, 0),
      af_n_eac  = ifelse(exporter %in% afc_countries & importer %in% afc_countries & !(exporter %in% eac_countries) & !(importer %in% eac_countries) & exporter != importer, 1, 0),
      eac_to_row = ifelse(exporter %in% eac_countries & !(importer %in% afc_countries) & exporter != importer, 1, 0),
      af_n_eac_to_row = ifelse(exporter %in% afc_countries & !(exporter %in% eac_countries) & !(importer %in% afc_countries) & exporter != importer, 1, 0),
      rr_n_eac = ifelse(!(exporter %in% afc_countries) & !(importer %in% afc_countries) & exporter != importer, 1, 0)
    ) %>%
    mutate(
...
# ----- Estimation (EAC model) -----
estimate_model <- function(df, industry_id, industry_descr) {
  m <- fepois(
    trade ~ cntg + col + dist + lang + rta + rr_n_eac +
      eac + eac_to_af + af_to_eac + af_n_eac + af_n_eac_to_row + eac_to_row |
      exp_year + imp_year,
    data = df,
    vcov = ~ pair_id
  )
  ...
  broom::tidy(m, conf.int = TRUE) %>%
    filter(term %in% c("eac_to_af", "af_to_eac")) %>%
...
write_csv(results, file.path("C:/Users/muham/EGEI Dissertation/afcfta/output/csv/eac_border_results.csv"))
...
# ----- Make the two plots -----
make_rank_plot(results, "eac_to_af", "graph_eac_to_af.pdf",
               ylab = "EAC_to_AfCFTA Border Effect",
               sig_only = FALSE,
               se_pctl_max = 0.95,
               ciwidth_pctl_max = 0.95,
               est_pctl_range = c(0.05, 0.95))

make_rank_plot(results, "af_to_eac", "graph_af_to_eac.pdf",
               ylab = "AfCFTA_to_EAC Border Effect",
               sig_only = FALSE,
               se_pctl_max = 0.95,
               ciwidth_pctl_max = 0.95,
               est_pctl_range = c(0.05, 0.95))
