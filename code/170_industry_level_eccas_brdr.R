# ====================== Industry-level analysis for ECCAS =======================
...
# ----- Data prep (merge + ECCAS border variables; ifelse for dummies, time-varying via as.integer(factor())) -----
prepare_data <- function(itpde_file) {
  ...
  df <- itpde %>%
    ...
    mutate(
      eccas = ifelse(exporter %in% ecc_countries & importer %in% ecc_countries & exporter != importer, 1, 0),
      eccas_to_af = ifelse(exporter %in% ecc_countries & importer %in% afc_countries & !(importer %in% ecc_countries) & exporter != importer, 1, 0),
      af_to_eccas = ifelse(importer %in% ecc_countries & exporter %in% afc_countries & !(exporter %in% ecc_countries) & exporter != importer, 1, 0),
      af_n_eccas  = ifelse(exporter %in% afc_countries & importer %in% afc_countries & !(exporter %in% ecc_countries) & !(importer %in% ecc_countries) & exporter != importer, 1, 0),
      eccas_to_row = ifelse(exporter %in% ecc_countries & !(importer %in% afc_countries) & exporter != importer, 1, 0),
      af_n_eccas_to_row = ifelse(exporter %in% afc_countries & !(exporter %in% ecc_countries) & !(importer %in% afc_countries) & exporter != importer, 1, 0),
      rr_n_eccas = ifelse(!(exporter %in% afc_countries) & !(importer %in% afc_countries) & exporter != importer, 1, 0)
    ) %>%
    mutate(
      eccas            = as.integer(factor(paste(eccas, year, sep = "_"))),
      eccas_to_af      = as.integer(factor(paste(eccas_to_af, year, sep = "_"))),
      af_to_eccas      = as.integer(factor(paste(af_to_eccas, year, sep = "_"))),
      af_n_eccas       = as.integer(factor(paste(af_n_eccas, year, sep = "_"))),
      eccas_to_row     = as.integer(factor(paste(eccas_to_row, year, sep = "_"))),
      af_n_eccas_to_row= as.integer(factor(paste(af_n_eccas_to_row, year, sep = "_"))),
      rr_n_eccas       = as.integer(factor(paste(rr_n_eccas, year, sep = "_"))),
...
# ----- Estimation (ECCAS model) -----
estimate_model <- function(df, industry_id, industry_descr) {
  m <- fepois(
    trade ~ cntg + col + dist + lang + rta + rr_n_eccas +
      eccas + eccas_to_af + af_to_eccas + af_n_eccas + af_n_eccas_to_row + eccas_to_row |
      exp_year + imp_year,
    data = df,
    vcov = ~ pair_id
  )
  ...
  broom::tidy(m, conf.int = TRUE) %>%
    filter(term %in% c("eccas_to_af", "af_to_eccas")) %>%
...
write_csv(results, file.path("C:/Users/muham/EGEI Dissertation/afcfta/output/csv/eccas_border_results.csv"))
...
# ----- Make the two plots -----
make_rank_plot(results, "eccas_to_af", "graph_eccas_to_af.pdf",
  ylab = "ECCAS_to_AfCFTA Border Effect",
  sig_only = FALSE,      
  alpha = 0.10,
  se_pctl_max = 0.95,    
  ciwidth_pctl_max = 0.95,  
...
make_rank_plot(results, "af_to_eccas", "graph_af_to_eccas.pdf",
               ylab = "AfCFTA_to_ECCAS Border Effect",
               sig_only = FALSE,
               alpha = 0.10,
               se_pctl_max = 0.95,    
               ciwidth_pctl_max = 0.95,  
